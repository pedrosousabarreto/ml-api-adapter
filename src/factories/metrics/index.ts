...
export class Metrics implements IDiFactory {
	...
	private _histograms:Map<string, Histogram>;
	...
	init(callback: (err?: Error) => void) {
		...
		// const options = {timeout: 5000, prefix: `${this._configs.app_name}`};
	...
	private _inject_routes(callback: (err?: Error) => void) {
		...
		router.get("/", this._handler_get_metrics.bind(this));

		// inject metrics endpoint
	...
	public get_histogram(name:string, help?:string, label_names?:string[]):Histogram {
